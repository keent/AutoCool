...
+(void)setRights{
	...
	args[i] = NULL;
	status=AuthorizationExecuteWithPrivileges(authorizationRef,[tool cString],0,args,&commPipe);
...
-(void) acFireTimer2:(NSTimer *)theTimer {
	...
	fFanTemp = [smcWrapper get_maintemp];
	
	if ([[defaults objectForKey:@"Unit"] intValue] != 0) { // if Fahrenheit convert to Celsius
		fFanTemp = (fFanTemp - 32) * (5/9);
	}
	
	iFanTemp = (int) fFanTemp;
	

	if (iFanTemp < 45)	{
		nsRPM = [NSNumber numberWithInt:2000];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
	else if (iFanTemp >= 45 && iFanTemp < 50)	{
		nsRPM = [NSNumber numberWithInt:2500];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
	else if (iFanTemp >= 50 && iFanTemp < 55)	{
		nsRPM = [NSNumber numberWithInt:3000];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
	else if (iFanTemp >= 55 && iFanTemp < 60)	{
		nsRPM = [NSNumber numberWithInt:3500];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}	
	else if (iFanTemp >= 60 && iFanTemp < 65)	{
		nsRPM = [NSNumber numberWithInt:4000];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
	else if (iFanTemp >= 65 && iFanTemp < 70)	{
		nsRPM = [NSNumber numberWithInt:4500];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
	else if (iFanTemp >= 70)	{
		nsRPM = [NSNumber numberWithInt:5000];
		[smcWrapper setKey_external:[NSString stringWithFormat:@"F%dMn",0] value:[nsRPM tohex]];
	}
}

-(void) acThreadMainMethod2	{
	...
	// Create a timer source here which interval is 10 seconds and changes fan based on temp
	NSDate *futureDate = [NSDate dateWithTimeIntervalSinceNow:1.0];
	NSTimer *myTimer = [[NSTimer alloc] initWithFireDate:futureDate 
												interval:10
												  target:self
												selector:@selector(acFireTimer2:)
												userInfo:nil 
												 repeats:YES];
	...
	while (!exitNow) {
		...
		// Run the run loop (sleeps until the timer is fired)
...
- (IBAction) setAutoCoolOn:(id) sender	{
	if (!autoCoolON){
	NSThread *acThread = [[NSThread alloc] initWithTarget:self
												  selector:@selector(acThreadMainMethod2)
													object:nil];
	gacThread = acThread;
	[acThread start];
	autoCoolON = YES;
